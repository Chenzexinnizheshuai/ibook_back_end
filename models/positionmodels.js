...
//获取全部数据
var alldata =(req,res,_query= {} )=>{
    // let _query = {} //查询条件 为空表示全部数据都进行查询，单个查询{'bookname','西游记'}
    return TheBook.find(_query).then((res)=>{
        return res
    }).catch((err) => { 
        return false
    }) 
...
//获取一页数据
var data =async (req,res,_query = {})=>{
    var {pagenum} = req.query;//页码
    pagenum = pagenum||1;
    var pagesize = 10;//一页中的数据数量
    var allpage =await alldata(req,res,_query)
    // let _query = {} //查询条件 为空表示全部数据都进行查询，单个查询{'bookname','西游记'}
    return TheBook.find(_query).sort({_id:-1}).skip((~~pagenum-1)*pagesize).limit(pagesize).then((res)=>{
